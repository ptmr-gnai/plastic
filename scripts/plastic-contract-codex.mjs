export const assertCodexCoreMethodDescriptions = ({ assert, descriptions }) => {
  ...
  assert(descriptions.connect.outputSchema?.required?.includes("connected"), "codex/connect output schema must require connected");
  assert(descriptions.connect.effects?.durableEvents?.includes("codex.connection.started"), "codex/connect must describe connection started event");
  assert(descriptions.initialize.outputSchema?.description?.includes("delegated Codex"), "codex/initialize output schema must describe delegated Codex result");
  assert(descriptions.initialize.effects?.durableEvents?.includes("codex.connection.initialized"), "codex/initialize must describe initialized event");
