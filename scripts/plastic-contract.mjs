...
import { assertContextualResourceAffordances, assertResourceAffordanceIntegrity, expectedSnapshotLinks, hasLinkAffordance, hasServiceAffordance } from "./plastic-contract-affordances.mjs";
...
await check("plastic/state", async () => {
  ...
  const description = await rpc("methods/describe", { id: "plastic/state" });
...
await check("plastic/methods", async () => {
  ...
  assertResourceAffordanceIntegrity({ assert, resources: state.resources, methodIds, methods: items, source: "state" });
...
await check("plastic/snapshot", async () => {
  ...
  assertArray(snapshot.resources, "snapshot.resources is not an array");
  ...
  assertResourceAffordanceIntegrity({ assert, resources: snapshot.resources, methodIds, methods, source: "snapshot" });
...
await check("runtime/runAuditAction description", async () => {
  ...
  assertRunAuditActionMethodDescription({ assert, description });
