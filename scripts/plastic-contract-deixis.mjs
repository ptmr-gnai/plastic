export const assertDeixisMethodDescriptions = ({ assert, descriptions }) => {
  ...
  assert(descriptions.resolveRef.outputSchema?.properties?.actions?.items?.required?.includes("method"), "deixis/resolveRef action schema must expose method");
  assert(descriptions.resolveRef.outputSchema?.properties?.verification?.items?.required?.includes("method"), "deixis/resolveRef verification schema must expose method");
  assert(descriptions.resolveRef.outputSchema?.properties?.lineage?.items?.required?.includes("id"), "deixis/resolveRef lineage schema must expose event ids");
  ...
  assert(descriptions.verifyRefAction.outputSchema?.properties?.matchedEvents?.items?.required?.includes("id"), "deixis/verifyRefAction matched event schema must expose ids");
  assert(descriptions.verifyRefAction.outputSchema?.properties?.refEvents?.items?.required?.includes("id"), "deixis/verifyRefAction ref event schema must expose ids");
  assert(descriptions.verifyRefAction.outputSchema?.properties?.timeline?.properties?.items?.items?.required?.includes("eventId"), "deixis/verifyRefAction timeline schema must expose event ids");
