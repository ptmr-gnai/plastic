...
export const assertControlLegibilityAndThemeProjection = async ({ methods, rpc }) => {
  ...
  assertCodexCoreMethodDescriptions({ assert, descriptions: Object.fromEntries(await Promise.all([["status", "codex/status"], ["defaults", "codex/defaults"], ["setDefaults", "codex/setDefaults"], ["request", "codex/request"], ["threadStart", "codex/threadStart"], ["turnStart", "codex/turnStart"], ["modelList", "codex/modelList"]].map(async ([key, id]) => [key, await rpc("methods/describe", { id })]))) });
