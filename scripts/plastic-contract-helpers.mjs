...
export const assertControlLegibilityAndThemeProjection = async ({ methods, rpc }) => {
  ...
  assertChatMethodDescriptions({ assert, descriptions: Object.fromEntries(await Promise.all([["messages", "chats/messages"], ["addButton", "chats/addButton"], ["injectUserMessage", "chats/injectUserMessage"], ["binding", "chats/getBinding"], ["bind", "chats/bindCodexThread"], ["start", "chats/startCodexThread"], ["create", "chats/createCodexChat"], ["interrupt", "chats/interrupt"], ["close", "chats/close"], ["send", "chats/sendToCodex"]].map(async ([key, id]) => [key, await rpc("methods/describe", { id })]))) });
