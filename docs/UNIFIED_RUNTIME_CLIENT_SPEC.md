...
Move from method catalog completeness to contextual action completeness. Contract-generated durable validation events must remain inspectable but should not leave active user-shaped projections behind. The next unification target is making `plastic/state`, `plastic/snapshot`, `agent/workbench`, `agent/orient`, visible refs, and extension resources expose enough HATEOAS actions that agents can choose likely next moves without hardcoding method IDs.
