...
Every method exposed through `plastic/methods` and `methods/describe` should eventually include availability. Runtime clients should also learn stable method, projection, transport, audit, extension, Deixis, and agent packet result shapes from RPC metadata. Open schemas should now be treated as explicit boundaries: empty input objects for no-input calls, resource affordance payload/schema descriptors, `deixis/evalDom` DOM-script results, `chats/interrupt`/`codex/*` Codex app-server passthrough results, and Codex alias inputs where Plastic is not the schema owner.
...
- The runtime/build HTTP transports share request helpers, RPC dispatch, method GET dispatch, `/host`, `/state`, `/methods`, `/capabilities`, `/snapshot`, `/self-test`, SSE event streams, CORS behavior, and error envelopes.
- `runtime/host`, `runtime/capabilities`, `runtime/auditStatus`, `runtime/runAuditAction`, `methods/describe`, `runtime/modules`, `agent/workbench`, `agent/orient`, `plastic/state`, and `plastic/snapshot` expose enough host/capability metadata for agents to learn which methods are available, degraded, or unavailable, inspect compact audit summaries, and run current audit remediation actions by id.
- `runtime/host`, `agent/workbench`, `agent/orient`, and `build/status` expose self-describing agent transports: HTTP RPC links/actions, a direct HTTP `/self-test` health link, and the MCP stdio `plastic_rpc` tool/action. These descriptors point back to the shared method registry and expose the same RPC input schema across HTTP actions, MCP tool metadata, and MCP call actions.
- `runtime.started` durably records the shared host descriptor, capability inventory, module inventory, and control plane. `plastic/selfTest` now verifies that live `runtime/host`, `runtime/capabilities`, and `runtime/modules` still match that durable startup descriptor and that runtime/build control-plane URLs are internally coherent.
- Runtime hosts no longer assume default control-plane ports during validation. `PLASTIC_RUNTIME_URL`, `PLASTIC_BUILD_URL`, `PLASTIC_RUNTIME_PORT`, `PLASTIC_BUILD_PORT`, `PLASTIC_VITE_URL`, and `VITE_DEV_SERVER_URL` are wired through Electron dev, headless dev, validation, launch probes, renderer event streams, visible UI status, runtime CORS, and diagnostics; headless and Electron validation can run beside a live default Electron app on alternate ports.
- The shared contract harness validates headless end to end, including state, methods, architecture-doc method parity, method descriptions for audit control, snapshot, capabilities, modules, panel lifecycle, extension scan/list, event streams, HTTP error contracts, build HTTP surfaces, and self-test.
- The shared contract harness cleans up generated extension/panel fixtures and tags raw, panel lifecycle, scaffold, and scan discovery/removal validation events with `validation:contract`.
- The shared contract harness now requires every method catalog entry to expose description, input schema, output schema, effects, reversibility, and directly invokable describe/invoke links. `methods/describe` links include `{ id }`, `rpc/call` links include `{ method }`, and the latest captured headless method baseline has 86 methods with zero missing metadata fields.
- Resource affordance integrity now applies to both links and actions: anything referencing an input-bearing method must provide concrete `input` satisfying required fields or an `inputSchema` that teaches agents how to call it.
- `plastic/state` and `plastic/snapshot` now expose active panels as individual contextual HATEOAS resources with concrete `input` payloads for panel read/control actions. Chat-shaped panel resources also expose concrete chat message actions.
- The shared contract harness now fails if state or snapshot has panel projections but no individual panel resource with invokable `panels/get`, `panels/rename`, and `panels/remove` affordances.
- `agent/workbench` and `agent/orient` expose the same focused panel read/control affordances when called with a panel context. The contract invokes both around `chat-main` and requires exact `panels/get`, `panels/rename`, and `panels/remove` inputs in recommended actions.
- `agent/workbench` and `agent/orient` expose validated control/navigation links for host, modules, self-test, audit status/action planning, control plane, and timeline. Input-bearing links carry concrete input or schemas, audit-status packets use the shared compact audit metadata shape including method-parity report metadata, and both the contract and self-test verify the expected link sets.
- `deixis/resolveRef` now exposes concrete panel resource links and read/control actions for visible refs with panel ownership. The Electron contract resolves an actual visible panel/chat ref and requires exact `panels/get`, `panels/rename`, and `panels/remove` inputs.
- `plastic/state` and `plastic/snapshot` now expose active extensions as individual contextual HATEOAS resources with concrete get, activate, verify, register-panel, fork, timeline, and method-description affordances.
- `plastic/state` and `plastic/snapshot` now expose active windows as individual contextual HATEOAS resources with concrete focus and scroll actions for projected panel ids.
- `pnpm plastic:contract-mcp-bridge` exercises MCP `tools/list` and `tools/call` through a shared validation client, proving `plastic_rpc` can call `runtime/auditStatus` and `runtime/auditActionPlan` through the same method registry.
- `pnpm plastic:method-parity` runs `plastic/selfTest` in both hosts, compares MCP tool metadata, compares agent transport links/actions/tools, compares module method ownership and required-capability declarations, compares method describe/invoke affordances, enforces shared capability status parity while permitting host-specific capability status differences, splits method documentation/schema/effect-contract drift, splits service-resource link/action and snapshot-link discovery drift, and verifies shared runtime health checks are green while permitting host-specific checks to differ. Shared checks now include runtime-started descriptor, capability inventory parity, module inventory parity, runtime host identity, audit status, and agent transport affordances. Its capture output records capability statuses and notes, and its console summary prints a capability-status map plus a machine-readable failure-count summary that also gates pass/fail; unified validation persists that report and asserts the saved summary total is zero, so agents can see which host capabilities are available, degraded, or unavailable without inferring from failures.
- Runtime health checks now treat required user/agent primitives as module contributions. `runtime-modules:map` verifies that core methods such as `plastic/state`, `plastic/methods`, `methods/describe`, `rpc/call`, `runtime/capabilities`, `runtime/host`, `runtime/modules`, `runtime/auditStatus`, `runtime/auditActionPlan`, `runtime/runAuditAction`, `panels/create`, and `plastic/selfTest` are present under their owning runtime modules instead of maintaining a detached required-method list.

`pnpm plastic:validate-unified` is the current automation-friendly validation command. It must run the shared headless runtime contract strictly, capture the headless method baseline, attempt Electron preflight and headed validation, compare Electron methods to headless when Electron is available, and emit a structured degraded report when Electron cannot start in the current host environment.

`pnpm plastic:audit-runtime-unification` is the full audit command for this workstream. It runs typecheck, guardrails, strict headless validation, strict Electron validation, and unified parity validation on explicit side-by-side validation ports by default, then prints one JSON summary and writes it to `.plastic/tmp/runtime-unification-audit.json`. The artifact is self-describing: it includes `schemaVersion: 1`, `generatedAt`, `inProgress`, `expectedStepIds`, `checks`, and `expectedChecks`, and the runner validates those invariants before persisting the file. When unified validation passes, the audit summary embeds the persisted method-parity report path, mode, and failure-count summary under `runtimeUnification.methodParity`. Failed steps include a bounded output tail and focused diagnostic hints so `runtime/auditStatus` can explain the failure through RPC. `runtime/auditStatus` must expose both the raw summary and a compact `verdict` with `status`, headed/headless usability, method-parity mode and failure total, failure phase, machine-readable diagnosis, hints, next action, recent audit action completions with command/env and bounded output tails, and diagnostic actions with human-readable command text, structured `{ command, args, env }` run metadata, plus Plastic-native `method`/`input` invocation metadata; `runtime/auditStatus`, `runtime/auditActionPlan`, and `runtime/runAuditAction` output schemas must also describe compact method-parity metadata and recent audit action tails instead of generic objects. `runtime/auditActionPlan` must preserve the same compact method-parity status in its audit metadata across HTTP RPC and MCP bridge calls. While `inProgress` is true, the verdict status must be `running` rather than `failed`. `runtime/runAuditAction` must let an agent run a current diagnostic action by id through the shared command runner and append `runtime.auditAction.completed`, so diagnostics are controllable and durable from both headed and headless hosts. Electron diagnostics must include the command and diagnostic env used for each step, app-mode smoke with its own short timeout, launch mode/target, explicit app-path, package-main, compiled-bootstrap, child lifecycle, and bounded child file-handle breadcrumbs; if a minimal Electron app cannot enter main, the diagnosis should be `electron-app-mode-smoke-not-entered` and the verdict actions should include both `PLASTIC_ELECTRON_SKIP_APP_MODE_SMOKE=1 pnpm plastic:validate-electron` and the cheap launch-target probe. If Electron launches but the main bootstrap is not observed, the verdict actions should include both a package-launch comparator and a cheap launch-target probe that records executable, package main, compiled main, child command lines, relevant env, and direct-binary/CLI-wrapper/isolated-profile entry-marker probes without running the full validation loop. Port-binding diagnostics must work for default and alternate runtime/build ports. If strict Electron is unavailable, the audit still runs unified validation and reports `runtimeUnification.unified: "degraded"` while keeping `ok: false`.

## Practical Next Step
Move from affordance integrity to behavioral convergence: every shared non-visual method should have one runtime implementation, one durable event/effect contract, and one headed/headless contract path. Contract-generated durable validation events must remain inspectable but should not leave active user-shaped projections behind.
Proof:
1. `plastic/selfTest` stays green while reporting stable shared projection health, required runtime module contributions, live-vs-durable `runtime.started` host/capability/module parity, coherent control-plane URLs, the current audit-status verdict shape, projection resource affordance integrity, agent packet link/action/input health, and agent transport affordance health for HTTP RPC plus MCP `plastic_rpc`.
2. `pnpm plastic:validate-unified` fails if any method catalog item lacks required metadata, any resource/agent/transport affordance points to an input-bearing method without concrete input or schema, or a shared non-visual method diverges between headless and Electron.
