...
- `runtime/host`, `agent/workbench`, `agent/orient`, and `build/status` expose self-describing agent transports: HTTP RPC links/actions and the MCP stdio `plastic_rpc` tool/action. These descriptors all point back to the shared method registry rather than a separate client implementation.
