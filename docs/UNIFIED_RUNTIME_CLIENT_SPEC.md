...
- `plastic/state` now exposes active panels as individual contextual HATEOAS resources with concrete `input` payloads for panel read/control actions. Chat-shaped panel resources also expose concrete chat message actions.
- The shared contract harness now fails if state has panel projections but no individual panel resource with invokable `panels/get`, `panels/rename`, and `panels/remove` affordances.
...
Continue contextual action completeness beyond panel resources. Contract-generated durable validation events must remain inspectable but should not leave active user-shaped projections behind. The next unification target is making `plastic/snapshot`, `agent/workbench`, `agent/orient`, visible refs, and extension resources expose enough concrete HATEOAS actions that agents can choose likely next moves without hardcoding method IDs or payload keys.
