...
## Practical Next Step
Continue contextual affordance completeness by auditing the remaining projected resources, agent packets, and transport descriptors for exact method inputs or schemas. Contract-generated durable validation events must remain inspectable but should not leave active user-shaped projections behind.
Proof:
1. `plastic/selfTest` stays green while reporting stable shared projection health, required runtime module contributions, live-vs-durable `runtime.started` host/capability/module parity, coherent control-plane URLs, the current audit-status verdict shape, agent packet link/action health, and agent transport affordance health for HTTP RPC plus MCP `plastic_rpc`.
2. `pnpm plastic:validate-unified` fails if any method catalog item lacks required metadata or any resource, agent packet, or transport affordance points to an input-bearing method without concrete input or schema in either host mode.
