...
2. `pnpm plastic:method-parity` remains clean after repeated headless/Electron audit runs and includes `runtime-audit:status` and `agent-transports:affordances` in shared health checks.
3. `plastic/selfTest` stays green while reporting stable shared projection health, required diagnostics methods, live-vs-durable `runtime.started` host/capability/module parity, coherent control-plane URLs, the current audit-status verdict shape, and agent transport affordance health for HTTP RPC plus MCP `plastic_rpc`.
