...
Continue contextual action completeness by auditing the remaining projected resources for exact method inputs. Contract-generated durable validation events must remain inspectable but should not leave active user-shaped projections behind.

Proof:
1. `pnpm plastic:method-parity` remains clean after repeated headless/Electron audit runs, includes `runtime-audit:status` and `agent-transports:affordances` in shared health checks, and prints current capability statuses for the active host.
2. `plastic/selfTest` stays green while reporting stable shared projection health, required runtime module contributions, live-vs-durable `runtime.started` host/capability/module parity, coherent control-plane URLs, the current audit-status verdict shape, and agent transport affordance health for HTTP RPC plus MCP `plastic_rpc`.
3. `pnpm plastic:validate-unified` fails if any method catalog item lacks description, input schema, output schema, effects, reversibility, or method affordance links.
