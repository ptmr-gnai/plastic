...
4. `PLASTIC_ELECTRON_LAUNCH_PROBE_RUN=1 node scripts/plastic-electron-launch-probe.mjs` prints a bounded JSON launch-target, child command-line, launcher-comparison, and entry-marker report that agents can run before heavier Electron diagnostics.
