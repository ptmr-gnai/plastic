...
3. `plastic/selfTest` stays green while reporting stable shared projection health, required diagnostics methods, and the current audit-status verdict shape, including structured diagnostic action run metadata and recent action completions with bounded output tails.
