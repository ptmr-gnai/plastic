...
`pnpm plastic:audit-runtime-unification` is the full audit command for this workstream. It runs typecheck, guardrails, strict headless validation, strict Electron validation, and unified parity validation, then prints one JSON summary. If strict Electron is unavailable, the audit still runs unified validation and reports `runtimeUnification.unified: "degraded"` while keeping `ok: false`.
