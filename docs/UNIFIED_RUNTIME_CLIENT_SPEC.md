...
- `pnpm plastic:method-parity` runs `plastic/selfTest` in both hosts, compares MCP tool metadata, compares agent transport links/actions/tools, and verifies shared runtime health checks are green while permitting host-specific checks to differ.
...
3. `plastic/selfTest` stays green while reporting stable shared projection health, required diagnostics methods, the current audit-status verdict shape, and agent transport affordance health for HTTP RPC plus MCP `plastic_rpc`.
