...
3. `plastic/selfTest` stays green while reporting stable shared projection health, required diagnostics methods, and the current audit-status verdict shape, including structured diagnostic action run metadata and recent action completions with command/env plus bounded output tails.
4. `PLASTIC_ELECTRON_LAUNCH_PROBE_RUN=1 node scripts/plastic-electron-launch-probe.mjs` prints a bounded JSON launch-target, child command-line, launcher/profile comparison, and entry-marker report that agents can run before heavier Electron diagnostics.
5. Validation events remain durable and inspectable, but are identifiable as validation scope rather than user workspace state.
