...
- `runtime/host`, `agent/workbench`, `agent/orient`, and `build/status` expose self-describing agent transports: HTTP RPC links/actions, a direct HTTP `/self-test` health link, and the MCP stdio `plastic_rpc` tool/action. These descriptors all point back to the shared method registry rather than a separate client implementation.
...
- The shared contract harness now requires every method catalog entry to expose a description, input schema, output schema, effects, reversibility, and describe/invoke links. This applies to runtime, build, and extension-owned methods, including bundled chat extension methods. The latest captured headless method baseline has 86 methods and zero missing metadata fields.
...
- `agent/workbench` and `agent/orient` now expose the same focused panel read/control affordances when called with a panel context. The contract invokes both around `chat-main` and requires exact `panels/get`, `panels/rename`, and `panels/remove` inputs in their recommended actions.
...
Continue contextual action completeness beyond panel resources. Contract-generated durable validation events must remain inspectable but should not leave active user-shaped projections behind. The next unification target is making visible refs and extension resources expose enough concrete HATEOAS actions that agents can choose likely next moves without hardcoding method IDs or payload keys.
...
2. `pnpm plastic:method-parity` remains clean after repeated headless/Electron audit runs, includes `runtime-audit:status` and `agent-transports:affordances` in shared health checks, and prints current capability statuses for the active host.
3. `plastic/selfTest` stays green while reporting stable shared projection health, required runtime module contributions, live-vs-durable `runtime.started` host/capability/module parity, coherent control-plane URLs, the current audit-status verdict shape, and agent transport affordance health for HTTP RPC plus MCP `plastic_rpc`.
4. `PLASTIC_ELECTRON_LAUNCH_PROBE_RUN=1 node scripts/plastic-electron-launch-probe.mjs` prints bounded launch-target, child command-line, launcher/profile, and entry-marker diagnostics; `PLASTIC_VITE_URL=http://127.0.0.1:<vite> PLASTIC_RUNTIME_URL=http://127.0.0.1:<runtime> PLASTIC_BUILD_URL=http://127.0.0.1:<build> pnpm plastic:validate-electron` can run while the default Electron app remains live on `5173/7331/7332`.
5. `pnpm plastic:validate-unified` fails if any method catalog item lacks description, input schema, output schema, effects, reversibility, or method affordance links.
