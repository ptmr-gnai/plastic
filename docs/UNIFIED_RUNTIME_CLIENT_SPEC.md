...
- `agent/workbench` and `agent/orient` expose validated control/navigation links for host, modules, self-test, audit status/action planning, control plane, and timeline. Input-bearing links carry concrete input or schemas, and both the contract and self-test verify the expected link sets.
