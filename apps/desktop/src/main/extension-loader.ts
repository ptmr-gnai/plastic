...
interface ExtensionVerificationCheck {
  name: string;
  ok: boolean;
  message: string;
}

interface ExtensionVerificationReport {
  extensionId: string;
  ok: boolean;
...
const verifyExtension = async (workspaceDir: string, extension: PlasticExtension, panelId?: string) => {
      ...
      handler: (inputValue) =>
        Effect.promise(async () => {
          ...
          const event = await runPromise(
            eventStore.append(
              createEvent({
                type: report.ok ? "extension.verified" : "extension.verify_failed",
                payload: {
                  id: extension.id,
